 ...
 *
 */
public class DriveToEncoderDistanceHoldHeading extends Command implements PIDOutput, PIDSource {
    ...
    //dir is +/- 180
    public DriveToEncoderDistanceHoldHeading(Encoder encoder) {
    	super("DriveToEncoderDistanceHoldHeading");
    ...
    public DriveToEncoderDistanceHoldHeading(Encoder encoder, double dir, double target, double p, double i, double d, double tolerance) {
    ...
    public DriveToEncoderDistanceHoldHeading(Encoder encoder, double dir, double target, double p, double i, double d, double tolerance, double minSpeed, double maxSpeed, double minInput, double maxInput) {
    	super("DriveToEncoderDistance");
        requires(Robot.mecanumDrive);
    ...
    public void pidWrite(double output) {
    	...
    	if(this.cancel || this.isFinished()){ 
    		...
    		if(Math.abs(output) < this.minSpeed) {
    			...
    			if(output > 0.0) {
    				Robot.mecanumDrive.getRoboDrive().mecanumDrive_Cartesian(0.0, minSpeed, this.skewPidOutput.getPidOutputValue(), direction);
    			} else {
    				Robot.mecanumDrive.getRoboDrive().mecanumDrive_Cartesian(0.0, minSpeed * -1.0, this.skewPidOutput.getPidOutputValue(), direction);
    			}
    		} else {
    			Robot.mecanumDrive.getRoboDrive().mecanumDrive_Cartesian(0.0, output, this.skewPidOutput.getPidOutputValue(), direction);
    		}
    	}
    }
}
